...
<x-seller.layout>
    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                ...
                <h5 class="card-title">Hitung Biaya Tetap
                ...
                <form id="dynamicForm" action="{{ route('seller.tetap.store') }}" method="POST">
                    @csrf
                            ...
                            @foreach($tetap as $item)
                                <tr>
                                    ...
                                    <td class="text-center fw-medium">
                                        <form action="{{ route('seller.tetap.destroy', $item->id) }}" class="d-inline" onsubmit="return confirm('Apakah kamu yakin ingin menghapus item ini?')" method="POST">
                                            @csrf
                                            @method('DELETE')
    ...
    <script>
        ...
        function calculateTotal(element) {
            ...
            const totalCostInput = inputGroup.querySelector('input[name="total_biaya[]"]');
            totalCostInput.value = (quantity * price).toFixed(2);
        }

        tooltipIcon.addEventListener('click', function () {
                alert('Biaya yang tidak berubah terlepas dari tingkat produksi atau penjualan. Ini adalah biaya yang harus dibayar secara reguler, biasanya dalam periode waktu tertentubiaya tetap. Misal, sewa tempat, gaji.');
            });
    </script>
</x-seller.layout>
